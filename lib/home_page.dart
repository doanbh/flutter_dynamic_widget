...
import 'package:widget_demo_dynamic/demo_ui.dart';
...
class HomePage extends StatefulWidget with WidgetsBindingObserver {
  const HomePage({super.key, required this.title, required this.platform});
...
class _HomePageState extends State<HomePage> {
        ...
        TextButton(
          onPressed: () {
            if (currentTask?.status == DownloadTaskStatus.complete){
              ...
              Navigator.push(context, MaterialPageRoute(builder: (context) => DemoUIPage(path)));
            } else {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
  ...
  Future<void> _prepare() async {
    ...
    _items.add(ItemHolder(name: 'Demo'));
